"""
Wi-Fi Radar - GUI version (Windows only)
Minimalist dark-themed UI built with Tkinter (built into Python, no
extra installs needed). Shows the currently connected network, nearby
visible Wi-Fi networks ('netsh wlan show networks'), and previously
saved network profiles ('netsh wlan show profiles') with in-range status.
"""
...
# ---- Fully-qualified System32 paths ----------------------------------
# Loading/spawning these by bare name ("wlanapi.dll", "netsh") lets Windows'
# standard search order resolve them, which can include directories an
# attacker controls (CWE-427 / CWE-426). Pinning the absolute System32 path
...
def _clean(value: str) -> str:
    """Strip non-printable/control characters from adversary-influenceable
    text (e.g. a crafted SSID) before it's used or displayed, on top of the
    plain whitespace trim."""
...
# ---- WLAN API: force an active scan (what the Windows tray icon does) ----
...
def trigger_wifi_scan() -> bool:
    """Ask the Windows WLAN service to actively rescan for networks - the
    same action the taskbar Wi-Fi icon triggers. netsh only reads whatever
    is already cached, so without this it can lag behind what Windows
    itself sees. Returns True if at least one interface was told to scan.
    Fails silently (returns False) on any error or non-Windows platform."""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                for i in range(min(header.dwNumberOfItems, 32)):
...
def get_saved_profiles() -> list[str]:
    """Return Wi-Fi profile names Windows has saved (previously connected to),
    whether or not they're currently in range."""
    command = [NETSH_PATH, "wlan", "show", "profiles"]
    try:
        result = subprocess.run(command, capture_output=True, timeout=10, check=True)
...
def get_current_connection() -> dict | None:
    """Return details of the Wi-Fi network this device is currently
    connected to, via 'netsh wlan show interfaces'. Returns None if not
    connected to any network."""
    command = [NETSH_PATH, "wlan", "show", "interfaces"]
    try:
        result = subprocess.run(command, capture_output=True, timeout=10, check=True)
    except (FileNotFoundError, subprocess.CalledProcessError, subprocess.TimeoutExpired):
        return None
...
def get_saved_profile_password(profile_name: str) -> str | None:
    """Return the saved password (PSK) for a given Wi-Fi profile name, if
    Windows has one stored. Returns None if the profile has no saved key
    (e.g. open networks) or on any failure. This only ever reveals keys
    already stored locally on this machine for networks it previously
    connected to - it does not attempt to crack or discover anything."""
    command = [NETSH_PATH, "wlan", "show", "profile", f'name="{profile_name}"', "key=clear"]
    try:
        result = subprocess.run(command, capture_output=True, timeout=10, check=True)
    except (FileNotFoundError, subprocess.CalledProcessError, subprocess.TimeoutExpired):
        return None
...
def signal_tag(signal_str: str) -> str:
    try:
...
class ScannerApp(tk.Tk):
    ...
    def _build_layout(self):
        ...
        # ---- Footer (pack to bottom BEFORE the expanding body, so it's
        # always reserved space and never gets squeezed out) ----
    ...
    def _run_scan_pass(self):
        ...
        # Merge into results collected so far - a single netsh call only
        # returns whatever Windows has cached at that instant, and the
        # cache fills in over a few seconds, so re-querying a couple of
        # times a beat apart tends to surface networks a single snapshot
        # misses.
        for net in networks:
    ...
    def _on_saved_profile_double_click(self, event):
        ...
        if password:
            ...
            messagebox.showinfo(
                profile, "No saved password found (open network, or key not stored)."
            )

    def _update_connection_card(self, connection: dict | None):
        if connection:
